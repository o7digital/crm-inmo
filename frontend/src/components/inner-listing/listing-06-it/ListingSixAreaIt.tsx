...
const ListingSixAreaIt = () => {
    ...
    fetch("/api/properties?status=published")
      ...
      .then((data) => {
        if (data?.errors?.length) {
          ...
          setError("Non siamo riusciti a caricare le proprieta al momento.");
          setProperties([]);
          setLoading(false);
          return;
        }
  ...
  const typeOptionGroups = useMemo(() => {
    ...
    properties.forEach((prop) => {
      if (prop?.property_type) {
        ...
        if (normalized && !propertyTypes.has(normalized)) {
          const localized = localizePropertyTitle(prop.property_type.toString().trim(), "it");
    ...
    if (operationTypes.size) {
      const operationOptions = Array.from(operationTypes.values()).map((operation) => ({
        ...
        label: operation === "venta" ? "Vendita" : operation === "renta" ? "Affitto" : operation,
      }));

      const hasSale = operationTypes.has("venta");
      if (hasSale) {
        operationOptions.push({ value: "operation:compra", label: "Acquisto" });
      }
  ...
  const locationOptions = useMemo(() => {
    ...
    return Array.from(locations.values()).sort((a, b) =>
      normalizeText(a).localeCompare(normalizeText(b), "it")
    );
  }, [properties]);
  ...
  return (
    <div className="property-listing-six pt-200 xl-pt-150 pb-200 xl-pb-120">
      <div className="container">
        <h2 className="mb-40 text-center">Proprieta disponibili</h2>

        <div className="listing-filters mb-40">
          <div className="row g-3 align-items-end">
            <div className="col-12 col-md-4 col-lg-3">
              <label htmlFor="listing-type-filter-it" className="form-label fw-500">
                Tipo / Operazione
              </label>
              <select
                id="listing-type-filter-it"
            ...
            <div className="col-6 col-md-4 col-lg-2">
              <label htmlFor="listing-min-price-it" className="form-label fw-500">
                Prezzo min.
              </label>
              <input
                id="listing-min-price-it"
                type="number"
            ...
            <div className="col-6 col-md-4 col-lg-2">
              <label htmlFor="listing-max-price-it" className="form-label fw-500">
                Prezzo max.
              </label>
              <input
                id="listing-max-price-it"
                type="number"
                className="form-control"
            ...
            <div className="col-12 col-md-6 col-lg-3">
              <label htmlFor="listing-location-filter-it" className="form-label fw-500">
                Localita
              </label>
              <input
                id="listing-location-filter-it"
                className="form-control"
                value={inputs.location}
                onChange={handleInputChange("location")}
                list="listing-location-options-it"
                placeholder="Digita la localita o lascia vuoto per tutte"
              />
              <datalist id="listing-location-options-it">
        ...
        {loading && <p className="text-center">Caricamento proprieta...</p>}
        {!loading && error && <p className="text-center text-danger">{error}</p>}
        {!loading && !error && properties.length === 0 && <p>Nessuna proprieta trovata.</p>}

        <div className="row">
          {!loading && !error && properties.length > 0 && filteredProperties.length === 0 && (
            <div className="col-12">
              <p className="text-center">Nessuna proprieta corrisponde ai filtri selezionati.</p>
            </div>
          )}
