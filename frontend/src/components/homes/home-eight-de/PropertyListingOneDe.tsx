...
const PropertyListingOneDe = () => {
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        <p className="fs-20">Immobilien werden geladen...</p>
      </div>
    );
  }
  ...
  if (!properties.length) {
    return (
      <div className="text-center py-5">
        <p className="fs-20">Noch keine empfohlenen Immobilien.</p>
      </div>
    );
  }
      ...
      <div className="container container-large">
        <div className="position-relative">
          <div className="title-one text-center mb-25 lg-mb-10 wow fadeInUp">
            <h3>Ausgewählte Immobilien</h3>
            <p className="fs-22 mt-xs">Entdecken Sie unsere Top-Objekte zum Kauf und zur Miete.</p>
          </div>
                  ...
                  <div className="listing-card-four overflow-hidden d-flex align-items-end position-relative z-1">
                    ...
                    <div className="property-info tran3s w-100 p-20 bg-white position-absolute bottom-0">
                      <div className="d-flex align-items-center justify-content-between">
                        <div className="pe-3">
                          <Link href={`/de/property/${item.id}`} className="title fw-500 tran4s">
                            {item.Title || "Unbenannte Immobilie"}
                          </Link>
                          <div className="address tran4s">
                            {item.Address || "Adresse nicht verfügbar"}
                          </div>
                        </div>
                        <Link href={`/de/property/${item.id}`} className="btn-four inverse">
                      ...
                      <ul className="style-none feature d-flex flex-wrap align-items-center justify-content-between mt-3">
                        ...
                        <li>
                          <strong className="color-dark fw-500">
                            {item.Price ? `$${item.Price.toLocaleString("en-US")}` : "Kontaktieren Sie uns"}
                          </strong>
                          <span className="fs-16">{item.Operation || "Kauf/Miete"}</span>
                        </li>
                      </ul>
                    </div>
                  </div>
          ...
          <div className="text-center mt-100 md-mt-60">
            <Link href="/de/listing_06" className="btn-eight">
              <span>Alle Immobilien ansehen</span> <i className="bi bi-arrow-up-right"></i>
            </Link>
          </div>
        </div>
      </div>
...
export default PropertyListingOneDe;
